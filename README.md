...
```bash
pip install PyMuPDF pdfplumber layoutparser tqdm
```

2. Ensure the latest `grobid` server (v0.8 or later) is running.

3. Use the asynchronous script [`codes/pdf_to_json_hybrid.py`](./codes/pdf_to_json_hybrid.py) to combine
page-level text extraction with metadata from `grobid` and produce a single JSON file.  
Additional parsers such as **Marker** or **Nougat** can be integrated by extending this script:
