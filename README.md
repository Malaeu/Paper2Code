...
## 🧠 Methodology Adaptation Feature

Paper2Code now includes a powerful adaptation feature that allows you to take the methodology from a scientific paper and apply it to your own dataset with different variables. This is particularly useful when you want to:

1. Apply proven methodologies to new domains
2. Replace demographic variables (e.g., race) with different stratification variables (e.g., gender)
3. Maintain methodological rigor while working with your own data

### Using the Adaptation Feature
...
The script will:
1. Create a sample dataset if none exists, or use your own dataset
2. Automatically analyze your dataset structure with our intelligent mapping system
3. Generate a variable mapping tailored to your data and the paper methodology
4. Allow you to review and customize the mapping
5. Create an adaptation plan based on the original paper's methodology
6. Generate fully adapted code that works with your data

### Automatic Dataset Analysis

Our new intelligent mapping system:
- Automatically detects your dataset structure (CSV, Parquet, Excel, JSON)
- Analyzes column names, data types, and sample values
- Uses AI to propose the most appropriate variable mappings
- Generates a complete mapping file that you can review and edit
...
The adaptation works in four phases:
1. **Mapping**: Analyzing your dataset and creating intelligent variable mappings
2. **Planning**: Analyzing the paper and creating a roadmap for adaptation
3. **Analysis**: Examining each component and determining necessary changes
4. **Coding**: Generating the actual adapted code for your dataset

### Example: From Race-Specific to Gender-Specific Models

Here's a sample of an automatically generated mapping that adapts race-specific heart failure prediction models to gender-specific models:
...
The resulting code maintains all the methodological rigor of the original paper while working with your variables and dataset structure.

---

## 🌐 MCP Integration for Enhanced AI Capabilities

Paper2Code now supports Model Context Protocol (MCP) integration, enabling advanced AI capabilities through various services.

### MCP Servers Available

- **filesystem**: Access to local papers and repository files
- **github**: Integration with GitHub repositories
- **brave-search**: Online search capabilities
- **memory**: Short-term memory storage
- **sequential-thinking**: Enhanced reasoning for complex tasks
- **puppeteer**: Web browsing capabilities
- **context7**: Knowledge integration
- **mcp-deepwiki**: Access to structured knowledge

### Using MCP with Paper2Code
...
Edit `.cloudcode/modelcontext.config.json` to add your specific API keys and tokens.

3. **Access in Enhanced Pipeline**:
The enhanced pipeline can utilize MCP services when available, falling back to standard processing when unavailable.

### Benefits of MCP Integration

- **External Knowledge**: Access latest research papers and documentation
- **GitHub Integration**: Compare with existing implementations
- **Web Search**: Find relevant resources and references
- **Enhanced Reasoning**: Better analysis of complex machine learning models
