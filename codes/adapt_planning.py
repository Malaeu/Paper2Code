#!/usr/bin/env python3
"""
Adaptation Planning Module

This module is responsible for creating an adaptation plan based on the paper
and the target dataset. It serves as the first phase in the two-phase approach.
"""
...
def generate_plan(client, paper_json, dataset_info, variable_mapping, gpt_version):
    """
...
def main():
    ...
    parser.add_argument("--mapping_file", help="Path to the variable mapping JSON file")
    ...
    if args.mapping_file:
        try:
            with open(args.mapping_file, 'r') as file:
                variable_mapping = json.load(file)
        except Exception as e:
            print(f"Error loading variable mapping: {e}")
            sys.exit(1)
    elif "variable_mapping" in config:
        variable_mapping = config["variable_mapping"].get("original_to_adapted", {})
    else:
        # Generate mapping if dataset path is provided
        if dataset_info.get("path"):
            dataset_analysis = analyze_dataset(dataset_info["path"], dataset_info.get("format", "csv"))
            variable_mapping = generate_mapping({}, dataset_analysis)
    
    # Create OpenAI client
    client = create_openai_client()
    
    # Generate the plan
    plan = generate_plan(client, paper_json, dataset_info, variable_mapping, args.gpt_version)
